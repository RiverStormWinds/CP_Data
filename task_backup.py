    ...
    def getCase(self, index, rec):
        try:
            ...
            sql = """
                ...
                ORDER BY sx_cases_collection_detail.CASE_INDEX,sx_cases_detail.STEP
                """ % (collection_id)
                        ...
                        for j in param_data_id_list:
                            run_result_list.append(
                                "('%s','RUN_TYPE_IMMI',%d,%d,%d,%d,%d,%d,'STATE_UNTEST','%s','%s')" % (
                                    ...
                                    int(rec1['CASES_ID']), int(rec1['CASES_PARAM_DATA_ID']), int(j), rec1["CASE_NAME"],
            ...
            #     try:
            #         if cases_tree_ids:
            #             if rec1['CASES_ID'] in cases_tree_ids.keys():
            #                 rec1['TREE_DIRECTORY_ID'] = cases_tree_ids[rec1['CASES_ID']]
            #     except Exception as e:
            #         pass
            #     cases_list.append(rec1)
            ...
            if len(ds) <= 0:
                ...
                try:
                    for index, rec in enumerate(ds):  # 将任务(案例集)进行循环，将任务内部案例集进行分别分发
                        judje_key, msg = self.judje_key(rec)  # 使用judje_key进行判断案例是否打上标记
    ...
    def judje_key(self, rec):
        ...
        try:
            ...
            for i in ds_list:
                ...
                if cases_id == "" or cases_id == None or cases_id == "null" or cases_id == "(null)":
                    continue
                sql_case = "select PRIORITY from sx_cases where cases_id = {}".format(cases_id)

                ds = cF.executeCaseSQL(sql_case)
                if ds and ds[0]['PRIORITY']:
                    return 1, ""
                else:
                    continue
            print u'优先执行判断 -->  end'
    ...
    def process_queue_msg(self, run_record_info, server_ids, runtaskid, system_id, case_list, in_param_legal):
        ...
        if in_param_legal:
            try:
                ...
                for case_distibute_list in new_case_distibute_list:
                    root_case_distibute_dict = case_distibute_list.pop(0)
                    for case_distibute in case_distibute_list:
