...
class TestAgentMonitorSvc(win32serviceutil.ServiceFramework):
    _svc_name_ = "TestAgent Monitor Service"
    _svc_display_name_ = u"TestAgent 监控服务"
    # 服务描述
    _svc_description_ = u"用于监控时溪TestAgent的后台服务"
    ...
    def _getLogger(self):
        ...
        logger = logging.getLogger('[TestAgentMonitorSvc]')
        if getattr(sys, 'frozen', False):
            application_path = os.path.dirname(sys.executable)
        elif __file__:
            application_path = os.path.dirname(__file__)

        handler = logging.FileHandler(os.path.join(application_path, "TestAgent-Monitor-service.log"))
    ...
    def main(self):
        ...
        try:
            ...
            init_file = os.path.join(application_path, "TestAgentMonitorSvc.ini")
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        ...
        servicemanager.PrepareToHostSingle(TestAgentMonitorSvc)
        servicemanager.StartServiceCtrlDispatcher()
    else:
        win32serviceutil.HandleCommandLine(TestAgentMonitorSvc)
