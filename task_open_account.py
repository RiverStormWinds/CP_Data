...
class task(threading.Thread):
    ...
    #             # 开始写入接口日志
    #             if system_id in (
    #                     "142891090059622AV2", "142890425731984KH9", "1428908551835S0NXP", "14288901937052FXJA"):
    ...
    def getCase(self, index, rec):
        try:
            ...
            sign = 0
            # 已经在页面点击的时候下载
            # self.generate_account_group_xml(ADAPTER_TYPE_SYSTEM_ID, sign)
            c["collection_name"] = ds1[0]['COLLECTION_NAME']
            # 获取告警信息
            # sx_cases_collection_run_task 通过TASK_ID字段是否为空来判断是定时任务还是立即执行任务
            # 如果是定时任务，需要根据TASK_ID 来获取告警信息
            ...
            if ADAPTER_TYPE_SYSTEM_ID == '14774643386617P4VW':
                sql = """
                    SELECT
                    sx_cases.CASE_NAME,
                    ...
                    FROM
                        ...
                        ON (sx_cases_param_data.CASES_ID = sx_cases_detail.CASES_ID)
                        INNER JOIN sx_cases 
                    ...
                    ORDER BY sx_cases.CASE_NAME,sx_cases_detail.STEP
                    """ % (collection_id)
            else:
                sql = """
                    SELECT
                    ...
                    AND
                        sx_cases_collection_detail.COLLECTION_ID = %d
                    ORDER BY sx_cases_collection_detail.CASES_ID
                    """ % (collection_id)
            ...
            for rec1 in ds1:
                # if ADAPTER_TYPE_SYSTEM_ID == '14774643386617P4VW':
                #     if rec1['FUNCID']:
                #         func_ids.append(rec1['FUNCID'][0:3])
                ...
                for j in param_data_id_list:
                    run_result_list.append("('RUN_TYPE_IMMI',%d,%d,%d,%d,%d,%d,'STATE_UNTEST','%s')" % (
                        ...
                        int(rec1['CASES_PARAM_DATA_ID']), int(j), rec1["CASE_NAME"]))
            # 更新配置文件标签的属性 ZHLCAutoTestSetting.xml
            cF.updateZHLCAutoTestSettingXml(ADAPTER_TYPE_SYSTEM_ID, runtask_id, "")
            if run_result_list == []:
            ...
            for rec1 in ds1:
                ...
                # value_list.append(
                #     "(%d,%d,'%s',%d,'%s',%d,%d,%d,%d,%d,'%s',%d,'%s','%s','%s','%s','%s','%s',%d,%d,'%s',%d,'%s')" % (
                #         rec1['RUN_RECORD_ID'], rec1['PARAM_DATA_ID'], \
                #         rec1['RUN_TYPE'], rec1['TASK_ID'], rec1['SERVER_IP'], rec1['COLLECTION_ID'],
                #         rec1['CASES_DETAIL_ID'], rec1['COLLECTION_DETAIL_ID'], \
                #         rec1['CASES_ID'], rec1['CASES_PARAM_DATA_ID'], rec1['RESULT'], rec1['STEP'], rec1['FUNCID'],
                #         script_name, rec1['ADAPTER_TYPE'], MySQLdb.escape_string(rec1['PRE_ACTION']),
                #         MySQLdb.escape_string(rec1['PRO_ACTION']), rec1['PARAM_DATA'], int(rec1['EXPECTED_VALUE']),
                #         int(rec1['ACCOUNT_GROUP_ID']), rec1['SERVER_TYPE'], int(rec1['PARAM_INDEX'])),rec1['STEP_NAME'])

                # 因为DB2 不支持replace into的方式，所以，临时采用先删除，然后再插入的方式 fixme
                # if gl.g_ZHLCSystemSetting['TestCaseDBType'] == 'DB2':
                if self._mqserver._case_db_type == 'DB2':
                    sql = '''
                    ...
                    '''
                    if rec1['PRE_ACTION']:
                        # Logging.getLog().debug("cbs rec1['PRE_ACTION'], rec1['CASES_DETAIL_ID']...%s,%s"%(str(rec1['PRE_ACTION']),rec1['CASES_DETAIL_ID']))
                        try:
                            ...
                            exc_info = cF.getExceptionInfo()
                            Logging.getLog().error(exc_info)
                            Logging.getLog().error("前置数据有误, 请检查")
                    if rec1['PRO_ACTION']:
                        try:
                            ...
                            exc_info = cF.getExceptionInfo()
                            Logging.getLog().error(exc_info)
                            Logging.getLog().error("后置数据有误, 请检查")
                    crs.execute(sql, (rec1['PARAM_DATA_ID'], rec1['RUN_TYPE'], rec1['TASK_ID'], rec1['SERVER_IP'],
                ...
                sql = '''
                   SELECT
                    ...
                    WHERE 
                        sx_run_record.task_id = %d
                    ORDER BY
                        sx_cases.CASE_NAME, sx_run_record.STEP
                ''' % (runtask_id)
            else:
                sql = '''
                       SELECT
                        ...
                        WHERE 
                            sx_run_record.task_id = %d
                        ORDER BY
                            sx_run_record.CASES_ID,sx_run_record.STEP,sx_run_record.FUNCID
                    ''' % (runtask_id)
            ...
            cases_ids = []
            for rec1 in ds1:
                cases_id = rec1['CASES_ID']
                cases_param_data_id = rec1['CASES_PARAM_DATA_ID']
                cases_ids.append(cases_id)
                # cal_list.append((cases_id, cases_param_data_id))
            # 集合去除重复
            # cal_list = list(set(cal_list))
            case_num = len(list(set(cases_ids)))
    ...
    def getCasesList(self):
        ...
        try:
            ...
            if len(ds) <= 0:
                ...
                try:
                    for index, rec in enumerate(ds):  # 将多个任务(案例集)进行循环
                        # ------------------------------------------------------------
                        # 经过循环，则拿到每一个案例集
                        # 进行判断，首先判断system_id
                        #       则判断案例是否携带 tag_id 标签，
                        ...
                        #                   调用成功，返回开户结果
                        # ------------------------------------------------------------
                        sql = "select distinct(system_id) from sx_cases_collection where collection_id in " \
                              "(select distinct(collection_id) from sx_cases_collection_run_task " \
                              "where collection_run_task_id = %s)" % (rec['COLLECTION_RUN_TASK_ID'])  # 查询system_id

                        env_sys_id = cF.executeCaseSQL(sql)

                        if env_sys_id == '1428908551835S0NXP':  # 清算的系统id随后修改

                            # --------------------------------------------------------
                            #
                            # 进行开户操作
                            # 使用rabbitmq_rpc调用方式进行异步调用，及返回结果
                            # 此处进行rabbitmq_rpc方式异步调用
                            #
                            # --------------------------------------------------------
                            sql_account = "select s.cases_id, s.case_name, t.attribute_id, t.content " \
                                          "from sx_cases s " \
                                          "inner join sx_tag_relation tx on s.cases_id = tx.cases_id " \
                                          "inner join sx_tag t on t.attribute_id = tx.attribute_id " \
                                          "where s.cases_id in (" \
                                          "select cases_id from sx_cases_collection_detail " \
                                          "where collection_id in (" \
                                          "select collection_id from sx_cases_collection_run_task " \
                                          "where collection_run_task_id = %s))" \
                                          % (rec['COLLECTION_RUN_TASK_ID'])  # 通过run_task_id定位到标签

                            account_open = cF.executeCaseSQL(sql_account)  # 拿到标签标签字段及相关案例id

                            ret, msg = self.open_account(account_open)  # 进行开户操作
                            if ret < 0:
                                result = self.open_account_fail_operation()  # 如果开户失败进行失败后操作
                                print result

                            ret, msg = self.procedure_main(index, rec)  # 处理任务(案例集)相关的主要逻辑
                            if ret < 0:
                                return ret, msg
    ...
    def open_account(self, account_info):  # 开户代码具体实现

        print u"开户信息 --> ", account_info

        return 0, ''

    def open_account_fail_operation(self):
        return u'开账户失败操作'

    def procedure_main(self, index, rec):
        judje_key = self.judje_key(rec)  # 使用judje_key进行判断案例是否打上标记
        if judje_key:  # 如果有，则拿到所有前置，先进行前置的执行
            ret, pre_dict_all, cases_list = self.get_pre_list(index, rec)  # 拿到前置
            if ret:  # 前置成功拿到
                ...
                if tar == 0:  # 说明前置执行成功，继续执行后面的案例本身及案例后置
                    ...
                    carry_dict = cases_list
                    carry_case = self.chg_carry_dict(carry_dict)
                    ret, msg = self.redis_exec(carry_case)  # 任务写入redis
                    if ret < 0:
                        return ret, msg
            ...
            ret, carry_dict = self.getCase(index, rec)  # 拿到需要发送的案例信息
    ...
    def judje_key(self, rec):
        ...
        try:
            ...
            ds_list = cF.executeCaseSQL(sql)
            # print 'ds_list --> ', ds_list
            for i in ds_list:
                sql_case = "select PRIORITY from sx_cases where cases_id = {}".format(i['CASES_ID'])
                ds = cF.executeCaseSQL(sql_case)
                if ds[0]['PRIORITY']:
                    return 1
                else:
                    continue
    ...
    def rabbit_exec(self, carry_dict):
        try:
            ret, msg = self.new_task_distribute(carry_dict['runtask_id'], carry_dict['system_id'],
                                                carry_dict['run_record_id_list'], carry_dict['cases_list'],
                                                carry_dict['c[agent_id]'])
            if ret < 0:
                return ret, msg
    ...
    def get_pre_list(self, index, rec):  # 此函数是用来取出前置数据
        """
        {
            'CASES_ID': 123038,
            'CASE_NAME': u'12008306\u65b0\u589e\u8bc4\u8bba\u4eba\u5458\u6210\u529f',
            'STEP': 1,
            'ACCOUNT_GROUP_ID': 25,
            'COLLECTION_DETAIL_ID': 104562,
            'STEP_NAME': u'',
            'RUN_RECORD_ID': 7413665,
            'COLLECTION_ID': 5593,
            'PRE_ACTION': u'',
            'PARAM_INDEX': 1,
            'FUNCID': u'12008306',
        ...
        """
        try:
            cases_list = self.getCase(index, rec)  # fixme
            all_list = cases_list[1]['cases_list']
            case_ptr = []
            for i in all_list:
                case_dict = {}
                cases_id = i['CASES_ID']  # 拿到当前案例的id
                sql_str = 'select PRIORITY from sx_cases where cases_id = {}'.format(cases_id)  # 拿到案例的优先执标识符
                tag = cF.executeCaseSQL(sql_str)
                if tag[0]['PRIORITY'] == 1:

                    pre_action = i['PRE_ACTION']
                    case_dict['pre_action'] = pre_action  # 前置

                    case_dict['cases_id'] = i['CASES_ID']  # 案例id

                    step = i['STEP']  # 拿到当前案例的step
                    case_dict['step'] = step

                    case_name = i['CASE_NAME']
                    case_dict['case_name'] = case_name

                    system_id = cases_list[1]['system_id']
                    case_dict['system_id'] = system_id

                    account_group_id = i['ACCOUNT_GROUP_ID']  # 账户组id
                    case_dict['account_group_id'] = account_group_id

                    funcid = i['FUNCID']
                    case_dict['funcid'] = funcid  # funcid

                    cmdstring = i['CMDSTRING']
                    case_dict['cmdstring'] = cmdstring

                    run_record_id = i['RUN_RECORD_ID']
                    case_dict['run_record_id'] = run_record_id

                    step_name = i['STEP_NAME']
                    case_dict['step_name'] = step_name

                    case_ptr.append(case_dict)
            if case_ptr:
                return 1, case_ptr, cases_list[1]
            else:
                return 0, 0, 0
        except:
            exc_info = cF.getExceptionInfo()
            ...
            try:
                ...
                for task_id in curr_task_list:
                    ...
                    for run_record_id in case_result_dict.keys():
                        ...
                        param = (result['result'],
                                 MySQLdb.escape_string(result['cmdstring'].replace("&bfb&", "#").replace("&bft&", "=")),
                                 result['log'].replace("'", "''"), str(result['run_user']),
...
where TASK_ID = %s group by CASES_ID,RESULT) c group by c.CASES_ID ) cc where cc.num =1) and result = 'STATE_PASS'
                ...
                for task_id in task_record_id_set_dict.keys():
                    if len(task_record_id_set_dict[task_id]) == 0:
                        ...
                        if task_info_dict[task_id]["timed_task_flag"] == 0:
                            report_title = u"报告/%s//%s/%s/手动/%s" % (
                                task_info_dict[task_id]["collection_name"],
                                str(pass_count),
                                str(fail_count),
                        ...
                        try:
                            # 上传日志
                            f = XFer(self._mqserver._setting["FTPServer"]["IP"], 'agent', gl.gl_ftp_info['FTPServer']['PASSWORD'], '.',
    ...
    def new_task_distribute(self, runtaskid, system_id, run_record_id_list, cases_list, agent_list):
        """
        下发任务到队列
        :param runtaskid:
        :param system_id:
        ...
        :return:
        """
        ret, case_distribute_list = self.process_queue_msg(runtaskid, system_id, cases_list)
        if ret < 0:
            return ret, case_distribute_list

        try:
            count = int(len(case_distribute_list))
            agent_list.sort()
            ...
            for agent_id in agent_list:
                ...
                try:
                    ...
                    f = open("public_new_task_msg_%s"%str(time.time()), "w+")
                    f.write(str(public_new_task_msg))
                    f.close()
                    exc_info = cF.getExceptionInfo()
                    Logging.getLog().error(exc_info)
            ...
            for i in xrange(count):
                ...
                try:
                    case_distribute_list_str = json.dumps(case_distribute_list[i], ensure_ascii=False)
                except:
                    f = open("case_distribute_list_%s" % str(time.time()), "w+")
                    f.write(str(case_distribute_list[i]))
                    f.close()
                    exc_info = cF.getExceptionInfo()
                    Logging.getLog().error(exc_info)
                    Logging.getLog().info("case_distribute_list[i] error")
                    case_distribute_list_str = str(case_distribute_list[i])
                try:
    ...
    def process_queue_msg(self, runtaskid, system_id, case_list):
        """
        处理成适合入队列的用例下发消息
        :param case_list:
        :return:
    ...
    def sql_comt_copy(self, sum_agent_n, task_agent_n, n):
        try:
            sql = "update SX_AGENT_INFO set STATE = 'STATE_FREE', CURR_TASK_NUM = %s, UN_FINISH_CASES_NUM = %s where AGENT_CODE = '%s'" % (
            task_agent_n, sum_agent_n, n)
            ds = cF.executeCaseSQL_DB2(sql)
        except:
            exc_info = cF.getExceptionInfo()
                ...
                if is_trigger == True:
                    ...
                    if task_warning_id == "":
                        sql = '''insert into sx_cases_collection_run_task 
                            ...
                            int(rec['COLLECTION_ID']), int(rec['TEST_SYSTEM_ID']), rec['AGENT_ID'], int(rec['TASK_ID']))
                    else:
                        sql = '''insert into sx_cases_collection_run_task 
                                (COLLECTION_ID,
                                TEST_SYSTEM_ID,
                    ...
                    Logging.getLog().debug(sql)
                    cF.executeCaseSQL(sql)
                    sql1 = "select count(*) from sx_cases where DISABLEFLAG='0' and SYSTEM_ID='%s'" % (rec['SYSTEM_ID'])
                    ds1 = cF.executeCaseSQL(sql1)
    ...
    def processCancelTask(self):
        ...
        try:
            ...
            # if len(ds) > 0 and str(ds[0]['CANCEL_FLAG']) == '1':  # 用户取消了任务
            #     Logging.getLog().info(u"侦测到用户取消任务%d" % runtaskid)
            #     self._cases_task_dict[runtaskid]['cases_list'] = []
            #
            #     self._cases_task_send_uncomfirm_dict[runtaskid]['cases_list'] = []  # fixme 不管是否还需要收到应答了。
            #
            #     # 直接取消任务
            ...
            #             datetime.datetime.strptime(self._cases_task_dict[runtaskid]['run_end_time'],
            #                                        '%Y-%m-%d %H:%M:%S') - datetime.datetime.strptime(
            #                 self._cases_task_dict[runtaskid]['run_start_time'], '%Y-%m-%d %H:%M:%S')), int(runtaskid))
            #     Logging.getLog().debug(sql)
            #     cF.executeCaseSQL(sql)
            #
            #     self._cases_task_dict[runtaskid]['run_status'] = 'STATE_FINISH'
...
def main():
    ...
    时溪自动化测试平台-Test Agent Server v2.0 2018.10.25 alpha-7
    (上海时溪信息技术有限公司）
    ''')
    cF.readConfigFile()
